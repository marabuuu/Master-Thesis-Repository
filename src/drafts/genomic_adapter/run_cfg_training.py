...
Simpler than GDA and faster to converge since the gradient signal from
...
Usage (standalone, for debugging):
    python -m src.genomic_adapter.run_cfg_training --config src/config.yaml
"""
...
from .run_training import _build_config, _validate_cohort_coverage
...
def run_cfg_training(cfg: Dict[str, Any], verbose: bool = True) -> None:
    ...
    # Validate zip coverage for both cohorts before allocating GPU memory.
    ...
    # Checkpoint every val interval or every save_every_samples, whichever is longer.
    ...
    # max_steps counts micro-batches (automatic_optimization=False, accum=1 in Trainer).
    ...
    trainer = pl.Trainer(
        ...
        gradient_clip_val=None,       # CfgBackboneLitModel clips manually
        accumulate_grad_batches=1,    # manual accumulation via is_last_accum
    ...
    # Resume from a previous CFG run if last.ckpt exists in this experiment's autoenc dir.
    # Do NOT resume from the GDA run's checkpoint — different architecture.
    # backbone_ckpt_path is loaded in CfgBackboneLitModel.__init__, not here.
